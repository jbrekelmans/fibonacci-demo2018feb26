...
app.get('/', (req, res) => {
  ...
  if (n > 999n) {
    res.status(400).type('text').send('Query string parameter "n" must be at most 999');
    return;
  }
  ...
  res.status(200).type('text').send(`The ${n}${suffix} Fibonacci number is ${result}!`);
});

app.get('/healthcheck', (req, res) => {
  res.status(200).type('text').send(`I'm fine!`);
});
